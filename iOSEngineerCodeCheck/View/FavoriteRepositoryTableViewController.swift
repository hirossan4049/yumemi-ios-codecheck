...
final class FavoriteRepositoryTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let repo = self.presenter?.favoritedCoreDataRepositories[indexPath.row] else {
            return cell
        }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        guard let vc = storyboard?.instantiateViewController(identifier: "RepositoryDetailView") as? RepositoryDetailViewController else {
            return
        }
        guard let repository = self.presenter?.getRepository(index: indexPath.row) else {
            return
        }
